...
AI 에이전트(Gemini)와 협업하여 커밋을 생성할 때, `run_shell_command` 도구의 한계로 인해 멀티라인 커밋 메시지가 올바르게 적용되지 않는 문제가 발생할 수 있습니다.

이 문제를 해결하고 일관된 커밋 프로세스를 유지하기 위해 다음 워크플로우를 따릅니다. 이 섹션은 커밋을 **실행**하기 위한 임시 파일 생성 워크플로우를 다루며, 커밋 메시지를 **아카이빙**하는 규칙은 10번 섹션을 참고하세요.

1.  **AI의 커밋 메시지 생성**: 사용자가 커밋을 요청하면, AI는 제안된 전체 커밋 메시지를 마크다운 파일로 생성합니다.

2.  **파일 위치 및 이름 규칙**:
    -   **디렉토리 구조**: 생성된 파일은 `.commit_message/` 아래에 아카이빙 규칙(10.1)과 유사하게 연도별, 주간별로 저장됩니다. 이 디렉토리는 `.gitignore`에 등록되어 버전 관리되지 않습니다.
        -   **1단계 (연도별)**: `[YYYY]/` (예: `.commit_message/2025/`)
        -   **2단계 (주간별)**: `[YYYYMMDD(월)]-[YYYYMMDD(일)]/` (예: `20251110-20251116/`)
    -   **파일명**: `YYYYMMDD_커밋_요약.md` 형식을 따릅니다. 커밋 요약은 영어 소문자와 하이픈(-)으로 작성됩니다. (예: `20251114_update-commit-workflow.md`)
    -   **최종 경로 예시**: `.commit_message/2025/20251110-20251116/20251114_update-commit-workflow.md`

3.  **사용자의 커밋 실행**:
    -   AI는 생성된 파일의 경로를 사용자에게 알려줍니다.
    -   사용자는 해당 파일의 내용을 확인 후, `git commit -F <생성된_파일_경로>` 명령어를 사용하여 직접 커밋을 실행합니다.

이 방식을 통해 AI의 제안을 활용하면서도 최종적인 커밋 제어권은 사용자가 갖게 되어, 정확하고 안정적인 히스토리 관리가 가능합니다.