...
2. **참고 문서 확인**: `read_file` - (선택) 작업 관련 문서나 가이드가 있다면 내용을 파악
3. **git 상태 확인**: `git_status` - 현재 git 상태 파악
4. **staged 변경사항 분석**: `git_diff_staged` - 커밋 대상 파일들의 변경 내용 확인
5. **커밋 히스토리 참조**: `git_log` - 프로젝트의 기존 커밋 메시지 패턴 분석
6. **사고 과정 정리**: `sequential-thinking` - 변경사항 분석 및 커밋 메시지 구성
...
이 섹션은 AI 에이전트와 협업하여 커밋을 생성하고 관리하는 두 가지 주요 워크플로우를 정의합니다. 목적에 따라 적절한 절차를 따르십시오.

#### 12.1 이 저장소(git-commit-generator) 자체 커밋

이 저장소의 코드나 문서를 수정하고 커밋할 때 사용하는 워크플로우입니다. `run_shell_command`의 한계를 극복하고 안정적인 커밋 메시지를 적용하기 위해 임시 파일을 사용합니다.

1.  **메시지 생성**: AI는 커밋 메시지를 생성하여 `.commit_message/[YYYY]/[주간]/` 디렉토리에 저장합니다.
    -   **경로 예시**: `.commit_message/2026/20260119-20260125/20260121_docs-update.md`
    -   이 디렉토리는 `.gitignore`에 포함되어 버전 관리되지 않습니다.
2.  **커밋 실행**: 사용자는 AI가 제공한 파일 경로를 사용하여 커밋을 실행합니다.
    ```bash
    git commit -F .commit_message/2026/.../message.md
    ```

#### 12.2 외부 프로젝트 커밋 메시지 아카이빙

다른 프로젝트의 작업 내역을 이 저장소의 `commits/` 디렉토리에 아카이빙할 때 사용하는 워크플로우입니다.

1.  **단순 아카이빙**: 소량의 변경 사항은 `commits/[프로젝트]/...` 경로에 파일을 생성하고, **12.1의 워크플로우**를 통해 이 저장소에 커밋합니다.
2.  **복잡한 분할 커밋 (AI 협업)**: 방대한 변경 사항을 분석하여 논리적으로 나누어 아카이빙해야 할 경우, 심화 가이드를 따릅니다.
    -   **참조 문서**: `.agent/AI_COMMIT_WORKFLOW.md`
    -   **주요 절차**: AI 주도 분석 -> 스테이징 조작 -> 메시지 파일 생성 -> 사용자 검토 및 실행 반복
