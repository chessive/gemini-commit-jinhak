...
2.  **파일 위치 및 이름 규칙**:
    -   **디렉토리**: 생성된 파일은 프로젝트 루트의 `.commit_message/` 디렉토리에 저장됩니다. (이 디렉토리는 `.gitignore`에 등록되어 버전 관리되지 않습니다.)
    -   **파일명**: `YYYYMMDD_커밋_요약.md` 형식을 따릅니다. 커밋 요약은 영어 소문자와 하이픈(-)으로 작성됩니다. (예: `20251105_update-commit-workflow.md`)

3.  **사용자의 커밋 실행**:
    -   AI는 생성된 파일의 경로를 사용자에게 알려줍니다.
    -   사용자는 해당 파일의 내용을 확인 후, `git commit -F .commit_message/파일명.md` 명령어를 사용하여 직접 커밋을 실행합니다.
